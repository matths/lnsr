...
function match (patternParts, reqPathParts) {
  ...
  var params = {};
  var doesNotMatch = _.reduce(patternParts, function (doesNotMatch, patternPart, index) {
...
function path (pattern, middleware) {
  ...
  return function (req, res, next) {
    ...
    if (reqPathParts !== false) {
      ...
      if (_.isPlainObject(matched)) {
        if (!_.isPlainObject(req['params'])) {
          req.params = {};
        }
        req.params = _.extend(req.params, matched);
        matched = true;
      }
    }
