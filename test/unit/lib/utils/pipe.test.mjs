...
tap.test('pipe', tap => {
  tap.test('when using pipe', tap => {
    ...
    tap.equal(typeof formatCentsAsEuroPiped, 'function', 'pipe should return a function');
    tap.equal(formatCentsAsEuroPiped(1234567890), '12.345.678,90 €', 'should return correct result');
    tap.equal(formatCentsAsEuroPiped(1234567890), formatCentsAsEuro(1234567890), 'should return same result as called one by one');
    tap.end();
  });
  tap.end();
});
